...
import {message, Modal, Form, Input, Select, Table, DatePicker} from 'antd'
...
useEffect(()=>{
  const getAllTransactions=async()=>{
    try {
      ...
      setLoading(true)
      const res=await axios.post('/api/v1/transactions/get-transactions', {userid:user._id , frequency, selectedDate, type})
      setLoading(false)
...
const handleDelete=async(record)=>{
  ...
  const handleSubmit=async(values)=>{
    try {
      ...
      setLoading(true)
      if(editable){
        await axios.post('/api/v1/transactions/edit-transaction', {payload:{...values, userid:user._id},
      ...
      {loading && <Spinner/>}
        <div className="filters">
          ...
          <div>
            ...
            <button className='btn btn-dark' onClick={()=>setShowModal(true)}>Add More</button>
          </div>
        </div>
